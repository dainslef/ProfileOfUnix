...
toggleXmobarCmd = "dbus-send --session --dest=org.Xmobar.Control --type=method_call --print-reply " ++
...
myMouseBindings _ = Map.fromList $ [
...
myLayout = spacingRaw enbaleSmartBorder border
  enableScreenBorder border enableWindowBorder $ layoutHook def where
